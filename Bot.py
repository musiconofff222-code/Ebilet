...
def click_search_button(page, timeout=12000):
    ...
    # --- YENİ: submit sırasında yapılan network isteklerini yakala ---
    captured_responses = []

    def _on_response(response):
        try:
            url = response.url
            if any(k in url.lower() for k in ["search", "flight", "ticket", "external"]):
    ...
    for status, url in captured_responses:
        logging.info(f"📡 Yakalanan network isteği: {status} - {url}")

    # --- YENİ: ana sayfaya/anasayfa benzeri bir yere geri düştük mü tespit et ---
    landed_on_home = (
    ...
    if landed_on_home:
        redirect_info = "\n".join(f"• {status} — {url}" for status, url in captured_responses) or "Yok (network isteği yakalanamadı)"
        form_info = (
        ...
        send_telegram(
            ...
            f"<b>Yakalanan ilgili network yanıtları:</b>\n{redirect_info}"
        )
        raise Exception(
